...
└─────────────────────┬───────────────────────────────────────┘
           ...
           No                  Yes
            ...
            ▼                   ▼
    ┌───────────┐     ┌─────────────────┐
    │ Continue  │     │ Assess Revision │
    │ Next Task │     │ (LLM Call)      │
    └───────────┘     └────────┬────────┘
                               │
                     ┌─────────┴─────────┐
                     │                   │
                KEEP_PLAN           REVISE_PLAN
                     │                   │
                     ▼                   ▼
            ┌───────────────┐   ┌────────────────┐
...
**Original Task**: "Research the top 5 AI companies and create a comparison report"
...
**Obstacle Encountered**: Search tool returns incomplete data (only 3 companies found)

**Revision Assessment**: REVISE_PLAN

**Revised Plan**:
1. ✓ Search for top AI companies (completed)
2. Try alternative search queries for missing companies
3. Research all 5 companies with available data
4. Create comparison table
5. Write summary report with notes on data availability

---

## Future Enhancements

- **Intelligent obstacle detection**: Use LLM to analyze subtask output instead of keyword matching
- **Revision loop detection**: Track plan fingerprints to prevent circular revisions
- **Human approval workflow**: Add configurable checkpoints for plan changes
- **Partial progress preservation**: Capture and reuse partial work from interrupted subtasks
- **Max revision limit**: Add configurable limit to prevent runaway revisions
- **Revision confidence scores**: Let LLM express certainty about revision necessity
